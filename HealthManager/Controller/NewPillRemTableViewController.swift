//
//  NewPillRemTableViewController.swift
//  HealthManager
//
...
class NewPillRemTableViewController: UITableViewController ,UITextFieldDelegate,UITextViewDelegate, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    @IBAction func createReminder(_ sender: Any) {
        if myStringafd != "" && titleLabel.text != "" {
            ...
            let thisReminder = PillReminder(title: titleLabel.text!, time: myStringafd, dosage: adressOutlet.text!, icon: "dds", category: categ, note: noteLabel.text!)
