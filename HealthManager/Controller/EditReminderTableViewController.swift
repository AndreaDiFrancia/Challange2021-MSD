//
//  EditReminderTableViewController.swift
//  HealthManager
//
...
class EditReminderTableViewController: UITableViewController ,UITextFieldDelegate,UITextViewDelegate, UIPickerViewDelegate, UIPickerViewDataSource  {
    ...
    override func viewDidLoad() {
        ...
        titleLabel.text = DataManager.shared.reminderSegue?.title
        adressOutlet.text = DataManager.shared.reminderSegue?.adress
        dateLabel.text = DataManager.shared.reminderSegue?.dateTime
        noteLabel.text = DataManager.shared.reminderSegue?.note
        lblCategory.text = DataManager.shared.reminderSegue?.category
        myStringafd = (DataManager.shared.reminderSegue?.dateTime)!
    ...
    @IBAction func createReminder(_ sender: Any) {
        ...
        if myStringafd != "" && titleLabel.text != "" {
            ...
            let oldReminder = Reminder(title: (DataManager.shared.reminderSegue?.title)!, dateTime: (DataManager.shared.reminderSegue?.dateTime)!, adress: (DataManager.shared.reminderSegue?.adress)!, icon: (DataManager.shared.reminderSegue?.icon)!, category: (DataManager.shared.reminderSegue?.category)!, note: (DataManager.shared.reminderSegue?.note)!)
            
            APIManager.shared.deleteReminder(oldReminder)
            
            let thisReminder = Reminder(title: titleLabel.text!, dateTime: myStringafd, adress: adressOutlet.text!, icon: "dds", category: lblCategory.text!, note: noteLabel.text!)
            
            DataManager.shared.reminderSegue = thisReminder
