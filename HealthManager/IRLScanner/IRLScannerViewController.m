...
- (void)viewDidLoad {
    ...
    self.scan_button.clipsToBounds = YES;
    self.scan_button.layer.cornerRadius = 35.0;
}
...
- (IBAction)switchMode:(UIBarButtonItem *)sender {
    self.navigationItem.rightBarButtonItem.title = [self.navigationItem.rightBarButtonItem.title  isEqual: @"Manual"] ? @"Auto" : @"Manual";
    [self.cameraView setEnableBorderDetection:!self.cameraView.enableBorderDetection];
}
...
-(void)didLostConfidence:(IRLCameraView*)view {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [[weakSelf titleLabel] setBackgroundColor:[UIColor clearColor]];
    });
}
